...
if [ -x userbot_worker/.venv/bin/python ]; then
    ...
    nohup userbot_worker/.venv/bin/python userbot_worker/worker.py watch-shares --delay "${SHARE_DELAY:-5}" --refresh "${SHARE_REFRESH:-5}" > storage/logs/userbot-share-watcher.log 2>&1 &
fi
